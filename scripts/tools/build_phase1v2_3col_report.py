#!/usr/bin/env python3
"""3-column embedded HTML report: ORIGINAL | HIGHLIGHT | FLUX-EDITED.

Reuses the highlight cache from build_phase1v2_edit_report (or renders on
demand) and pulls the FLUX edited PNG from <in-dir>/2d_edits/{edit_id}_edited.png.

Only edits with edit_type ∈ {modification, scale, material, global} have a
flux output; deletion edits are still shown but the 3rd column is blank.

Usage:
    python scripts/tools/build_phase1v2_3col_report.py \
...
def stitch_triple(orig: np.ndarray, hl: np.ndarray,
                  edited: np.ndarray | None) -> np.ndarray:
    H, W = orig.shape[:2]
    if hl.shape[:2] != (H, W):
        hl = cv2.resize(hl, (W, H), interpolation=cv2.INTER_AREA)
    if edited is None:
        edited = np.full((H, W, 3), 245, dtype=np.uint8)
        cv2.putText(edited, "(no flux output)", (W // 8, H // 2),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (120, 120, 120), 1, cv2.LINE_AA)
    elif edited.shape[:2] != (H, W):
        edited = cv2.resize(edited, (W, H), interpolation=cv2.INTER_AREA)
    sep = np.full((H + 28, 6, 3), 180, dtype=np.uint8)
    a = np.vstack([_label("ORIGINAL", W), orig])
    b = np.vstack([_label("HIGHLIGHT", W), hl])
    c = np.vstack([_label("FLUX EDIT", W), edited])
    return np.hstack([a, sep, b, sep, c])
...
def main():
    ...
    ap.add_argument("--edits-subdir", default="2d_edits")
    ...
    out = args.out or (args.in_dir / "report_3col.html")
    hl_cache = args.in_dir / "_hl_edits"
    hl_cache.mkdir(parents=True, exist_ok=True)
    edits_dir = args.in_dir / args.edits_subdir
    ...
    n_edits_total = sum(type_counts.values())
...
<title>Phase 1 v2 — 3-col edit report</title><style>{style}</style></head><body>
<header>
  <h1>Phase 1 v2 — original | highlight | flux edit</h1>
  ...
  <div class="controls" id="filters">
    ...
    for fi, (f, j, edits) in enumerate(objs_meta):
        ...
        # SHARED seq across all flux types (matches parsed_to_edit_specs.py)
        flux_seq = 0
        ...
        for idx, e in enumerate(edits):
            ...
            # flux edited image (if any)
            edited = None
            if et in PREFIX:
                seq = flux_seq
                flux_seq += 1
                edited_path = edits_dir / f"{PREFIX[et]}_{obj_id}_{seq:03d}_edited.png"
                if edited_path.is_file():
                    edited = cv2.imread(str(edited_path), cv2.IMREAD_COLOR)

            if orig is not None and hl is not None:
                panel = stitch_triple(orig, hl, edited)
