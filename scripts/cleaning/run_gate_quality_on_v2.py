...
``partcraft.pipeline_v3.vlm_core.run_gate_quality`` directly.  Verdicts are
written back to v2's ``edit_status.json`` under ``gates.E`` (currently null
on every v2 run).  v2 module code is **not** modified.

Notes:
  * v2 and v3 share the on-disk object layout (``objects/<NN>/<obj_id>/``,
    ``phase1/parsed.json``, ``edits_3d/<edit_id>/preview_*.png``,
    ``edit_status.json``), so a vanilla ``PipelineRoot(root=v2_run)`` is a
    drop-in adapter.
  * ``run_gate_quality`` is async — wrapped with ``asyncio.run`` here.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--v3-config", type=Path, required=True,
                    help="a v3 config YAML (used for services.vlm + qc.thresholds)")
    ...
    asyncio.run(run_gate_quality(
        ...
        cfg=cfg, force=args.force, concurrency=args.concurrency,
    ))
    return 0
