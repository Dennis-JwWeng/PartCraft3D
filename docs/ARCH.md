...
#### 各编辑类型的 SLAT+SS+DINOv2 生产路径（2026-04-04 对齐）

Step4 产出 PLY + 初始 SLAT+SS（TRELLIS 生成），后处理 Phase 5 补充 `dino_voxel_mean` 并对 deletion 做完整重编码。

| 类型 | Step4 产出 | 后处理 Phase 5 | 最终 after.npz 来源 |
|------|-----------|---------------|-------------------|
| **modification / scale** | TRELLIS S1+S2 → SLAT+SS + `after.ply` | 渲染 `after.ply` → DINOv2 → **注入 `dino_voxel_mean`** | SLAT+SS 来自 TRELLIS，dino 来自 Phase 5 |
| **material / global** | TRELLIS S2-only → SLAT+SS + `after.ply` | 同上 | 同上 |
| **deletion** | `direct_delete_mesh()` → `before.ply` + `after.ply`（无有效 SLAT） | 渲染 `after.ply` → DINOv2 → SLAT encoder → SS encoder → **完整重写 `after.npz`** | 全部来自 Phase 5 重编码 |
| **addition** | 无独立产出 | 复制 deletion 的 before/after 互换 | 来自 deletion 的 Phase 5 产出 |
| **identity** | 无独立产出 | 硬链接同物体 `original.npz` | 来自原始物体预编码 |

`before.npz`（所有类型共享 `original.npz`）的 `dino_voxel_mean` 从 `slat_dir/{obj_id}_dino_voxel_mean.pt` 加载（由 `encode_into_SLAT` 产出），或 fallback 渲染 `before.ply`。
...
| **5** | **PLY 渲染 + DINOv2 + SLAT 重编码** | 所有有 `after.ply` 的编辑（见下方） | GPU（Blender + DINOv2 + SLAT enc + SS enc） |

Phase 5 的行为按编辑类型分两条路径：

- **Deletion after**：`after.ply` → Blender 40 views → voxelize → DINOv2 → SLAT encoder → SS encoder → **完整重写 `after.npz`**（4 个 key）
- **其他类型 after**（mod/scale/mat/glb）：`after.ply` → Blender 40 views → voxelize → DINOv2 → **仅注入 `dino_voxel_mean` 到已有 NPZ**（保留 TRELLIS 生成的 SLAT+SS）
- **Before 侧**（所有类型共享）：从 `slat_dir/{obj_id}_dino_voxel_mean.pt` 加载（由 `encode_into_SLAT` 产出），或 fallback 渲染 `before.ply`

推荐执行顺序：`--phase 1,3,4,5`（跳过已废弃的 Phase 2）。

```bash
# 旧格式转换 + addition/identity 回填 + DINOv2/SLAT 重编码
