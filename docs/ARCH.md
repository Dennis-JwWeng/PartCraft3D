...
- `dino_voxel_mean` `[N, 1024]` float16（可选）：多视角 DINOv2 ViT-L/14 patch 特征投影到体素后取平均，是 SLAT encoder 的输入中间产物

所有编辑类型均产出完整的 SS + SLAT 对，但生产方式因类型而异：

#### 各编辑类型的 SLAT+SS 生产路径

| 类型 | SLAT 来源 | SS 来源 | 实现位置 |
|------|----------|---------|---------|
| **modification / scale** | TRELLIS Stage-1+Stage-2 重新生成 | 随 TRELLIS 编辑一起产出 | `trellis_refine.py` `run_edit()` |
| **material / global** | TRELLIS Stage-2 only 重新生成（几何不变，仅改特征） | 随 TRELLIS 编辑一起产出 | `trellis_refine.py` `run_edit()` |
| **deletion** | PLY → Blender 渲染 → DINOv2 → SLAT encoder **重编码**（见下方详述） | 从重编码 SLAT coords → SS VAE 编码 | `migrate_slat_to_npz.py` `phase2_deletion()` |
| **addition** | 复制对应 deletion 的 before/after 并**互换** | 随 NPZ 一起复制 | `pipeline_step_3d.py` `_copy_addition_from_deletion()` |
| **identity** | 等于 original（训练时引用同一 NPZ） | 等于 original | `metadata.json` 引用 |

#### Deletion 详细流程（PLY → 渲染 → 重编码，2026-04-04 更新）

Deletion 的 after 状态走与原始物体**完全相同的编码路径**，确保 SLAT 特征在缺失零件的几何上下文中自洽：

```
after.ply → Blender Cycles 渲染 40 views → Open3D 体素化 64³ → DINOv2 特征提取 → SLAT encoder → SS encoder → after.npz
```

具体步骤：

1. **Step4 产出 PLY**：`TrellisRefiner.direct_delete_mesh()` 从 HY3D-Part mesh 真值中移除指定零件，导出 `before.ply`（完整）+ `after.ply`（删除后）
2. **后处理 Phase C 重编码**：对 `after.ply` 执行完整的渲染-编码链路
   - Blender Cycles GPU 渲染 40 views（512×512），产出 PNG + `transforms.json` + `mesh.ply`
   - Open3D 对 `mesh.ply` 体素化到 64³ 网格，产出 `voxels.ply`
   - DINOv2 ViT-L/14 提取 patch 特征，投影到体素坐标并跨视角平均 → `[N, 1024]`
   - SLAT encoder (`slat_enc_swin8_B`) 编码为 `[N, 8]` feats + `[N, 4]` coords
   - SS encoder (`sparse_structure_encoder`) 从 coords 构建占用网格并编码 → `[C, R, R, R]`
3. **before.npz**：仍使用原始物体的预编码 SLAT+SS（与其他编辑类型共享 `original.npz`）

Addition 无独立计算，仅将 deletion 的 `before.npz` ↔ `after.npz` 互换复制（硬链接优化）。

##### 渲染-编码链路复用（无需新增代码）

| 已有模块 | 复用的能力 |
|----------|-----------|
| `third_party/encode_asset/blender_script/render.py` | Blender Cycles 渲染，已支持 PLY 输入 |
| `third_party/encode_asset/render_img_for_enc.py` | `render()` + `voxelize()` 函数 |
| `third_party/encode_asset/encode_into_SLAT.py` | DINOv2 → SLAT encoder 完整链路 |
| `scripts/datasets/prerender_common.py` | `run_encode()` / `launch_multi_gpu_encode()` 多 GPU 调度 |

##### 为什么不能用 mask 过滤（旧方案废弃原因）

旧方案对原始 SLAT 做 `feats[keep]` / `coords[keep]` 子集化。问题是 SLAT 特征是在**完整物体上下文**中由 DINOv2 多视角聚合 + SLAT encoder 编码的，每个体素的特征隐含了对周围结构的感知。直接丢弃子集体素后，剩余体素的特征不再自洽，导致 TRELLIS Gaussian decoder 重建时出现严重的毛边与模糊。重编码方案让 DINOv2 在缺失零件的渲染图上重新提取特征，SLAT encoder 在新的体素结构上重新编码，产出的 latent 与删除后的几何完全一致。

##### 参数对齐

| 参数 | Deletion 渲染 | 原始物体预渲染 | 说明 |
|------|--------------|---------------|------|
| views | 40 | 150 | Deletion 用 40 views 即可，减少渲染时间 |
| resolution | 512 | 512 | 一致 |
| engine | CYCLES GPU | CYCLES | 一致 |
| voxel_size | 1/64 (64³) | 1/64 (64³) | 一致 |
| fov | 40° | 40° | 一致 |
| view sampling | sphere_hammersley | sphere_hammersley | 一致 |
...
- 每个 NPZ 含 `slat_feats [N,8]`、`slat_coords [N,4]`、`ss [8,16,16,16]`、`dino_voxel_mean [N,1024]` float16（可选）
- `dino_voxel_mean` 是 SLAT encoder 的输入：多视角 DINOv2 patch 特征投影到体素后取均值
...
for batch in loader:
    ...
    # 可选：dino_voxel_mean（NPZ 中有该字段时自动加载）
    before_dino = batch.get("before_dino")  # [total_N, 1024] 或 None
    after_dino  = batch.get("after_dino")   # [total_N, 1024] 或 None
```
...
- `dino_voxel_mean` 与 SLAT feats 共享同一稀疏布局（相同 N 和 coords），collate 后直接 `torch.cat` 拼接
...
`scripts/tools/run_postprocess.py` 整合预筛、VLM、SS 编码、DINOv2 特征提取、repack、cleaning 为五阶段管线：

| Phase | 功能 | 资源 |
|-------|------|------|
| **A** | 几何预筛：SLAT 编辑用 feats+coords 检查，PLY 编辑用 mesh 检查 | CPU |
| **B** | VLM 语义评分：SLAT 编辑走 TRELLIS 渲染，PLY 编辑走 Blender 渲染 | GPU + VLM API |
| **C** | 仅对 A+B 通过的子集编码 SLAT+SS：非 deletion 类型走 `migrate_slat_to_npz --include-list`；**deletion 类型走 PLY → Blender 渲染 40 views → DINOv2 → SLAT encoder → SS encoder 完整重编码** | GPU |
| **C2** | DINOv2 voxel 特征提取（`migrate_slat_to_npz --phase 5`）：对所有编辑的 `after.ply` 渲染 40 views → DINOv2 → 写入 `dino_voxel_mean [N,1024]` 到 NPZ；`before` 侧从 `slat_dir` 加载预存的 `_dino_voxel_mean.pt` 或 fallback 渲染 `before.ply` | GPU + Blender |
